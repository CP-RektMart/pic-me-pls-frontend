...
async function convertStringToFile(inputString: string): Promise<File> {
  ...
  const fileType = 'application/octet-stream' // Default MIME type for unknown files

  if (inputString.startsWith('data:')) {
    // It's a Base64 string, extract filename based on MIME type
    filename = extractFileNameFromBase64(inputString)
...
function extractFileNameFromBase64(base64String: string): string {
...
export default async function updateCitizenCard(
  ...
  try {
    ...
    const res = await fetch(
      `${process.env.BACKEND_URL}/api/v1/photographer/reverify`,
      {
        method: 'PATCH',
        headers: {
